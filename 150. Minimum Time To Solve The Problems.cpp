// Problem: https://www.codingninjas.com/codestudio/guided-paths/data-structures-algorithms/content/118621/offering/1377959?leftPanelTab=0

/*
    Time Complexity: O(N * log(Sum of the subjectsrray subjects)).
    Space Complexity: O(1).

    Where N and S denotes the number of subjects and sum of subject problems respectively.
*/

// Function to check whether it is possible to do k partitions such that maximum time <= mid.
bool checker(int k, vector<int> &subjects, int n, long long int mid) {
    ...
    while (i < n) {
        ...
        // If at any point count exceeds k, then simply return false. subjectss it is not possible to do k partitions.
        if (count > k) {
            return false;
        }
